...
export class OrderService {
    ...
    for (const item of cart.items) {
      ...
      if (dress) {
        const variant = dress.variants.find(
          (v) =>
            v.size._id.toString() === item.sizeId.toString() &&
            v.color._id.toString() === item.colorId.toString(),
        );

        if (variant) {
          variant.stock -= item.quantity;
          await this.dressService.update(item.dressId.toString(), {
  ...
  async processPayment(orderId: string, paymentMethod: any): Promise<Order> {
    console.log(
      `Processing payment for order ${orderId} with payment method:`,
      paymentMethod,
    );
    ...
    console.log(
      `Order ${orderId} status updated to confirmed after payment processing`,
    );
    return updatedOrder;
  }
  ...
  async trackOrderByPhone(phone: string): Promise<any[]> {
    try {
      ...
      const query = {
        'shippingAddress.phone': { $regex: formattedPhone, $options: 'i' },
      };
      ...
      if (!orders || orders.length === 0) {
        throw new NotFoundException(
          `Không tìm thấy đơn hàng với số điện thoại '${phone}'`,
        );
      }
  ...
  private formatOrderTracking(orderData: any, phone?: string): any {
    ...
    if (
      ...
      trackingInfo = {
        trackingNumber:
          ...
          `TK-${orderData.orderNumber?.substring(0, 6) || '000000'}`,
    ...
    return {
      orderCode:
        ...
        `ORD-${orderData._id?.substring(0, 6) || '000000'}`,
